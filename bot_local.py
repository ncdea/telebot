...
def send_about(message):
    ...
    with open('template_text/about.txt', mode='r', encoding='utf-8') as f:
        ...
        about = temp.substitute(
            STUDENT_NAME = "Nissa Corlidea",
            BATCH_ACADEMY = "Wizard",
            GITHUB_REPO_LINK = "https://github.com/ncdea" 
        )
...
unique_campaign = unique_campaign = df['campaign_id'].unique()
...
def send_summary(message):
     ...
     if selected_campaign_id in unique_campaign:
         ...
         df_campaign = df[df['campaign_id'].astype(str) == selected_campaign_id]
...
def send_plot(message):
     ...
     if selected_campaign_id in unique_campaign:
         ...
         df_campaign = df[df['campaign_id'].astype(str) == selected_campaign_id]
         df_plot = df_campaign[["spent","approved_conversion"]].groupby(by = df_campaign["age"]).sum()
         df_plot['cpc'] = df_plot["spent"] / df_plot["approved_conversion"]
         df_plot = df_plot.reset_index(drop=False, inplace=False)
         ...
         axes[0].bar(x=["30-34", "35-39", "40-44", "45-49"], height=[7559.179992, 5966.909990, 7252.099982, 18306.799997], color="#AE2024")
         axes[0].set_ylabel("Total Spent", fontsize=8)

         # second subplot: total approved conversion per age group
         axes[1].bar(x=["30-34", "35-39", "40-44", "45-49"], height=[166.0,78.0,88.0,162.0], color= "#000000")
         axes[1].set_ylabel("Total Approved Conversion", fontsize=8)

         # third subplot: average CPC per age group
         axes[2].bar(x=["30-34", "35-39", "40-44", "45-49"], height=[45.537229,76.498846,82.410227, 113.004938], color="#AE2024")
